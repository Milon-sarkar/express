    ...
    <table id="zero-config" class="table table-striped dataTable" style="width: 100%;" role="grid" aria-describedby="zero-config_info">
        <thead>
            <tr role="row">
                ...
                <th class="sorting" style="width: 15%;" tabindex="0" aria-controls="zero-config" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 162px;">Branch Manager name</th>
                <th class="sorting" style="width: 15%;" tabindex="0" aria-controls="zero-config" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 75px;">Branch name</th>
                ...
                <th class="sorting" style="width: 15%;" tabindex="0" aria-controls="zero-config" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 75px;">Password</th>
        ...
        <tbody> 
            @foreach($branchs as $k => $item)     
                <tr role="row">
                    ...
                    <td>{{$item->brance_id}}</td>
                    <td>{{$item->email}}</td>
                    <td>{{$item->phone}}</td>
                    <td>{{$item->password}}</td>
                    <td>{{$item->status==1?'Active':'Dactive'}}</td>                   
                    <td style="display: inline-block;">
                        <!-- <a class="btn btn-primary" href="/branch/{{$item->id}}"><i class="fas fa-user"></i></a> -->
                        <a class="btn btn-info" href="/branch/{{$item->id}}/edit"><i class="fas fa-user-edit"></i></a>
                        <form onsubmit="return confirm('Are you sure?')" action="/branch/delete/{{$item->id}}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('delete')          
